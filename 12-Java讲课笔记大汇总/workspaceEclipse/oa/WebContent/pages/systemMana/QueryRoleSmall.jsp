...
<script type="text/javascript">
	...
	function returnPopValue() {		
		...
		$(window.parent.document).find('#roleId').val(roleId);
		$(window.parent.document).find('#roleName').val(roleName);	
	...
	$(document).ready(function(){
		$('input[name="ids"]').click(function(){
			roleId = $.trim( $(this).parent().parent().find('td').eq(1).text() );
			roleName = $.trim( $(this).parent().parent().find('td').eq(2).text() );			
		});		
	});
</script>
</head>
<body>
	    	...
	    	<table class="table table-striped">
			  <tr>
			    ...
			    <th width="129" height="24">角色编号</th>
			    <th width="177" height="24">角色名称</th>   
			  </tr>
			
			  <c:forEach items="${requestScope.list }" var="role">
			  <tr>
			    <td height="24"><input type="radio" name="ids"></td>
			    <td height="24"><c:out value="${role.roleId }"/></td>
			    <td height="24"><c:out value="${role.roleName }"/></td>   
			  </tr>
			  </c:forEach>
			</table>
