...
<script type="text/javascript">
	...
	function del(mesId) {
		...
		if (bool) {
			...
			location.href= "<%=request.getContextPath() %>/message/delete.action?messageId="+mesId+"&flag=3"
		}
	}
</script>
  ...
  <div class="row" style="min-height:400px;"> 
    ...
    <div class="span8">
      ...
      <table class="table table-striped">
              <tr>
                <th><a href="<%=request.getContextPath() %>/message/queryRead.action?flag2=4">留言编号</a></th>
                <th><a href="<%=request.getContextPath() %>/message/queryRead.action?flag2=3">发件人id</a></th>
                <th><a href="<%=request.getContextPath() %>/message/queryRead.action?flag2=3">发件人姓名</a></th>
                <th><a href="<%=request.getContextPath() %>/message/queryRead.action">收件时间</a></th>
                <th>操作</th>
              </tr>
             <c:forEach items="${list }" var="message">
              <tr>
              <c:if test="${message.M_READ == 1 }">
                <td>${message.MESSAGE_ID }(未读)</td>
              </c:if>
              <c:if test="${message.M_READ == 2 }">
              	<td>${message.MESSAGE_ID }(已读)</td>
              </c:if>
